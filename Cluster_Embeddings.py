...
def t_SNE(embeddings,speaker_labels, num_dimensions = 2):
    ...
    plt.title('t-SNE plot for speaker embeddings trained with triplet loss on VGG-Vox (256 dimensional embedding)', fontsize=24)
    ...
    fig, _ = plt.subplots()
    fig.set_size_inches(22, 12)
    plt.scatter(x=coords["x"], y=coords["y"], c=y_kmeans, s=50, cmap='magma')
    #plt.setp(legend.get_title(), fontsize=20)
    # Adds annotations one by one with a loop
    for line in range(0, coords.shape[0]):
    ...
    plt.title('K-means clustering of speaker embeddings trained with triplet loss on VGG-Vox (256 dimensional embedding)', fontsize=24)
    plt.savefig('k-means.png')
    plt.show()

    fig, _ = plt.subplots()
    fig.set_size_inches(22, 12)
    plt.scatter(x=coords["x"], y=coords["y"], c=spectral_labels, s=50, cmap='magma')
    plt.show()
...
def coverage_and_purity(df, method='k-means'):
    """
    coverage_and_purity computer the coverage and purity for every cluster
    :param df: dataframe returned by clustering function
    :param method:
    :return: average coverage and purity of all clusters
    """
    ...
    for i in list(set(clusters)):
        ...
        cluster_count = len(df[df[method]== i])
        print("the cluster count is ", cluster_count)
        ...
        label_count = len(df[df.labels == cluster_label])
        purity = purity + (correct_clusters/cluster_count)
        coverage = coverage + (correct_clusters/label_count)
    coverage = coverage/len(list(set(clusters)))
    purity = purity/len(list(set(clusters)))
    print(coverage)
    print(purity)
...
def main():
    ...
    parser.add_argument('--load-path',default='/home/lucas/PycharmProjects/Papers_with_code/data/models/VGG_Triplet_Modified'
                        ,type=str, help='path to load models from')
    parser.add_argument('--name', default='Cluster Embeddings', type=str,
                        help='name of experiment')
    parser.add_argument('--speakers', type=list, default=['FEO065', 'FEO066','MEE067','MEE068','MEO069'],
